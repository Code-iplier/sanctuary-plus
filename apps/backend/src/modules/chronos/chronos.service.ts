...
export class ChronosService {
  ...
  async getSummary(): Promise<ChronosSummary> {
    ...
    const activePatients =
      typeof health.active_patients === 'number'
        ? health.active_patients
        : patients.count;
  ...
  async predict(
    ...
    return this.fetchWithTimeout<ChronosPredictionResponse>(
      ...
      throw new HttpException(
        `Chronos prediction failed: ${err.message}`,
        HttpStatus.BAD_GATEWAY,
      );
    });
  }
  ...
  async getStreamStatus() {
    return this.fetchWithTimeout<ChronosStreamStatus>(
      `${this.baseUrl}/stream/status`,
    );
  }

  async controlStream(action: 'play' | 'pause' | 'restart') {
    return this.fetchWithTimeout<ChronosStreamStatus>(
      `${this.baseUrl}/stream/${action}`,
      {
        method: 'POST',
      },
    );
  ...
  private async fetchWithTimeout<T>(
    ...
    try {
      ...
      if (msg.includes('abort'))
        throw new HttpException(
          `Chronos request timed out after ${this.timeoutMs}ms`,
